...
class Member extends Controller
{
    ...
    function create_profile(Request $req){
        ...
        if($record_exist != 0)
        {
         ...
         $record_exist_pan=Members::where('pan_no',$pan)->where('status','yes')->count();
        if($record_exist_pan != 0)
        {
            $req->session()->flash('err','PAN number already register');
            return redirect('add_member');
        }
         $record_exist_email=Members::where('email',$req->email)->where('status','yes')->count();
        if($record_exist_email != 0)
        {
        ...
        $record_exist_user_name=Members::where('user_name',$user_name)->count();
        $count=0;
        while($record_exist_user_name>0){
            $count++;
          $user_name=date('ymdHi').$count;  
          $record_exist_user_name=Members::where('user_name',$user_name)->count();
        }
        $pass=md5('hx1234');
    ...
    function payment(Request $req){
     ...
     function payment_generate(Request $req){
       // dd($req->amount[0]);
        ...
        Members::where('id', session('members_id'))->update(array('binary_b_amount'=>$act_amount,'withdrawal_eligibility' => 'yes'));
            Tranjaction_master::insert(
                         array(
        ...
        $gst_amm=$amount-($amount*(100/(100+18)));
        $act_amount=round($amount-$gst_amm,2);
        Members::where('id', session('members_id'))->update(array('binary_b_amount'=>$act_amount));
             Tranjaction_master::insert(
                         array(
                 ...
                 for ($i=0; $i <count($req->amount) ; $i++) { 
                     ...
                     $pad_date=date('Y-m-d');
                       ...
                       DB::table('user_emi')->insert(
                             array(
                                    
                                    'user_id'  => session('members_id'), 
                                    'emi_master_id'  => '1', 
                                    'emi_amount'     =>   $req->amount[$i], 
                                    'emi_date'     => $req->date[$i], 
    ...
    function pad_emi(Request $req){
        ...
        Members::where('id',session('members_id'))->
            update(
             array(
                    'package_id'     =>   session('package_id'),
                    'withdrawal_eligibility'=>'yes'
                     
                  )
             ); 
            }
            DB::table('user_emi')->where('id',$req->id)->
            update(
             array(
                    'pad_status'     =>   'yes',
                    'pad_date'=> date('Y-m-d'),
                    'pad_amount'     =>  $req->amount ,
            ...
            $req->session()->flash('succ','Payment Successfully');
               return redirect('emi');
    }
    function busness_cal(){
        ...
        //dd($ref_package_id[0]->package_id);
       ...
       // goto a;
        Members::where('id', $refer_id)->where('level_eligibility','1')->update(array('direct' => $direct_comm_amount+$pre_direct_amount,'tot_earning'=>$pre_tot_earning+$direct_comm_amount,'wallet_bal'=>$pre_wallet_bal+$direct_comm_amount));
        //,'business_amount'=>$bv_amount+$pre_bv_amount
       ...
       // dd($act_amount);
        ...
        $member_dtl_array=Members::where('status','yes')->where('total_team_actual_value','>=','10000000')->select('total_team_actual_value','id','refer_id_input','executive','tot_earning','wallet_bal')->get();
        $queries = DB::getQueryLog();
        //dd($queries);
        //dd($member_dtl_array);
        foreach ($member_dtl_array as  $value) {
            $member_count_amm=Members::where('status','yes')->where('refer_id',$value->id)->select('total_team_actual_value','id','refer_id_input')->where('total_team_actual_value','>=','400000')->count();
           // dd($member_count_amm);
            if($member_count_amm>0){
                $executive_amm=($value->total_team_actual_value*2)/100;
                ...
                $total_team_actual_value=$value->total_team_actual_value;
                Members::where('id', $value->id)->where('level_eligibility','1')->update(array('executive' => $old_executive+$executive_amm,'tot_earning'=> $tot_earning+$executive_amm,'wallet_bal'=> $pre_wallet_bal+$executive_amm,'total_team_actual_value'=>'0','tot_business_amount'=>$total_team_actual_value+$tot_business_amount));
        ...
        $direct_join_count=Members::where('refer_id',$id)->count();
        $available_direct=DB::table('steady_income_master')->where('status','yes')->where('eligibility','=',$level)->select('performance')->orderBy('id', 'DESC')->first();
        $direct_join=$available_direct->performance;
        //dd($direct_join_count);
        if($direct_join_count >= $direct_join){
        ...
        $steady_credit_amount=round(($bv_amount*$steady_income_master_percentage)/100,2);
           ...
           //echo " Amount ".$steady_credit_amount ."</br>";
            ...
            $member_dtl_steady=Members::where('status','yes')->where('id', $id)->select('tot_earning','wallet_bal','steady')->get();
            $cur_tot_earning=$member_dtl_steady[0]->tot_earning;
            $pre_wallet_bal=$member_dtl_steady[0]->wallet_bal;
            $pre_steady_amount=$member_dtl_steady[0]->steady;
            //echo " Pre Amount ".$pre_steady_amount ."</br>";
        ...
        Members_tranjaction::insert(
            ...
            //add team busness amount
            $pre_mem_val=Members::where('id',$id)->select('business_amount','binary_b_amount','total_team_actual_value')->get();
            Members::where('id', $id)->update(array('business_amount'=>$bv_amount+$pre_mem_val[0]->business_amount,'binary_b_amount'=>$act_amount+$pre_mem_val[0]->binary_b_amount,'total_team_actual_value'=>$act_amount+$pre_mem_val[0]->total_team_actual_value));
            DB::table('team_business_total_amm')->insert(
             array(
                    
                    'member_id'     =>   $id, 
                    'business_amount'     =>   $act_amount, 
                    'date'     => date('Y-m-d'), 
                   )
             );
             //add team busness amount
    ...
    function reset(){
      Members::where('status', 'yes')->update(array('direct' => '0','executive'=>'0','steady'=>'0','binary_bal'=>'0','wallet_bal'=>'0','tot_earning'=>'0','business_amount'=>'0','tot_business_amount'=>'0','binary_b_amount'=>'0','binary_b_amount'=>'0','cf_left'=>'0','cf_right'=>'0','cappimg_amount'=>'0','total_team_actual_value'=>'0'));
      DB::table('members_tranjaction')->delete();
      DB::table('team_business_total_amm')->delete();
      DB::table('tranjaction_master')->delete();
      echo "done";  
    }
    //Binary Income Start
    function binary_cor_jobs(){
        ...
        foreach ($member_dtl_array as  $value) {
            ...
            $cur_tot_earning=$value->tot_earning;
            ...
            if($count2>=1 && $count1>=2){
               // $max_amount=Members::where('status','yes')->where('refer_id',$member_id)->select('id','binary_b_amount')->max('binary_b_amount');
                $max_amount_array=Members::where('status','yes')->where('refer_id',$member_id)->select('id','binary_b_amount')->orderBy('binary_b_amount','desc')->first();
                DB::connection()->enableQueryLog();
                $other_tot_sum_amount=Members::where('status','yes')->where('refer_id',$member_id)->where('id','!=',$max_amount_array->id)->sum('binary_b_amount');
                $queries = DB::getQueryLog();
                //dd($other_tot_sum_amount);
                //dd($max_amount);
                $max_amount=$max_amount_array->binary_b_amount+$cf_left;
                ...
                //dd($member_id);
                  Members::where('id', $member_id)->where('level_eligibility','1')->update(array('binary_bal' => $binary_bal+$credit_amm,'tot_earning'=>$cur_tot_earning+$credit_amm,'wallet_bal'=>$wallet_bal+$credit_amm,'cf_left'=>$left_cf_amount,'cf_right'=>$right_cf_amount));
    ...
    function get_tree($id,$str){
         ...
         foreach ($name_dtl as $value){
            ...
            if($count>0){
               // 
                $str=$str.'<li><span class="caret" style="white-space: nowrap;">'.$value->display_name.'</span>';
                $str=$str.' <ul class="nested" style="white-space: nowrap;">';
    ...
    */
    ...
    function refer_member()
    {
        ...
        $encrypted=$this->num2alpha($eid*10000000);
       return view('refer_member',['encrypted'=>$encrypted]);
    }
    function join(Request $req){
        $encrypted =$this->alpha2num($req->id);
        $decrypted = $encrypted/10000000;
         $count=Members::where('id','=',$decrypted)->where('status','=','yes')->count();
        return view('join_member',['encrypted'=>$encrypted,'count'=>$count,'token'=>$req->id]);
    }
    function join_profile(Request $req){
        ...
        $pass=md5('hx1234');
        $decrypted = ($this->alpha2num($req->token))/10000000;
        $new_member_id= Members::insertGetId(
             array(
                    'user_name'     =>  $user_name, 
                    'password'     =>   $pass, 
     ...
     function verify_email_otp_profile(Request $req)
    {
       ...
       if(session('email_otp')!=$req->email_otp){
        ...
        function emi()
        {
            ...
            $user_emi=DB::table('user_emi')->where('user_id',session('members_id'))->get();
             return view('emi',['user_emi'=>$user_emi]);
        }
        function rewards()
        {
        $eligibility_month_rew=0;
        $eligibility_year_rew=0;

        $this_month_start=date('Y-m-d',strtotime('first day of this month')) ;
        $this_month_end=date('Y-m-d',strtotime('last day of this month')) ;
        $this_year_first_day=date("Y-m-d",strtotime("January 1st"));
        $this_year_last_day=date("Y-m-d",strtotime("December 31st")); 
        ...
        $tot_amm_year=DB::table('team_business_total_amm')->where('member_id',session('members_id'))->whereBetween('date',[$this_year_first_day, $this_year_last_day])->sum("business_amount");
           //dd($tot_amm_year);
        $member_id_cal=session('members_id');
        ...
        if($tot_amm_month>$ratio_min){
            $eligibility_month_rew=1;
        }
        $ratio_year=(round($tot_amm_year,2)*40)/100;
        $ratio_min_year=DB::table('team_business_total_amm')->where('member_id',$refer_id[0]->refer_id)->whereBetween('date',[$this_year_first_day, $this_year_last_day])->sum("business_amount");
        if($ratio_min_year>$ratio_year){
            $eligibility_year_rew=1;
        }
        //dd($ratio_min_year);
           return view('rewards',['tot_amm_month'=>$tot_amm_month,'tot_amm_year'=>$tot_amm_year,'eligibility_month_rew'=>$eligibility_month_rew,'eligibility_year_rew'=>$eligibility_year_rew]);
        }
     function power(){
        $str='';
        $i=1;
        $member_id_cal=session('members_id');
        $max_amount_array=Members::where('status','yes')->where('refer_id',$member_id_cal)->select('id','binary_b_amount','display_name','user_name')->orderBy('binary_b_amount','desc')->first();
        if(!empty($max_amount_array)){
        $cf_left_right=Members::where('status','yes')->where('id',$member_id_cal)->select('cf_left','cf_right')->get();
        //dd($max_amount_array);
            
                //dd($max_amount_array->user_name);
                $str=$str.'<tr>';
                $str=$str.'<td>'.$i++.'</td>';
                $str=$str.'<td>'.$max_amount_array->user_name.'</td>';
                $str=$str.'<td>'.$max_amount_array->display_name.'</td>';
                $str=$str.'<td>'.$max_amount_array->binary_b_amount+$cf_left_right[0]->cf_left.'</td>';
                $str=$str.'</tr>';
            }

        return view('power',['str'=>$str]);
    }
     function weaker(){
        $str='';
        $i=1;
        ...
        $other_tot_sum_amount=Members::where('status','yes')->where('refer_id',$member_id_cal)->where('id','!=',$ex_id)->select('display_name','user_name','binary_b_amount')->get();
        $cf_left_right=Members::where('status','yes')->where('id',$member_id_cal)->select('cf_left','cf_right')->get();
        foreach ($other_tot_sum_amount as $value) {
       
                $str=$str.'<tr>';
                $str=$str.'<td>'.$i++.'</td>';
                $str=$str.'<td>'.$value->user_name.'</td>';
                $str=$str.'<td>'.$value->display_name.'</td>';
                $str=$str.'<td>'.$value->binary_b_amount+$cf_left_right[0]->cf_right.'</td>';
                $str=$str.'</tr>';
          }
      }
        return view('weaker',['str'=>$str]);
    }
    function invoice(){
